...
# 申请测试证书
echo "申请测试证书..."
~/.acme.sh/acme.sh --issue $domain_args --server letsencrypt --listen-v6 --standalone -k ec-256 --force --test
if [ $? -ne 0 ]; then
...
# 申请正式证书
echo "申请正式证书..."
~/.acme.sh/acme.sh --issue $domain_args --server letsencrypt --listen-v6 --standalone -k ec-256 --force
if [ $? -ne 0 ]; then
